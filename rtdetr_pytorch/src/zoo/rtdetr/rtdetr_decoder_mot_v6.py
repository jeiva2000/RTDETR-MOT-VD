...
class TransformerDecoderLayerMOT_v2(TransformerDecoderLayer):
    ...
    def forward(self,
        ...
        if return_attn:
           return tgt, attn_weights, self_attn_weights, embeds_track, embeds_det
        else:
            if self.deformable:
...
class TransformerDecoderMOT_track(TransformerDecoder):
    ...
    def forward(self,
        ...
        masks = []
        for i, layer in enumerate(self.layers):
            mask = torch.sigmoid(mask_head[i](output)).squeeze(0)
            masks.append(mask)
            output, attn_weights, self_attn_weights, embeds_track, embeds_det = layer(output, None, memory,None, None, attn_mask=mask,query_pos_embed=query_pos_embed,return_attn=True, cross=False)
            #print('output:',output)
        ...
        return output, boxes[-1], scores[-1], attn_weights, self_attn_weights, embeds_track, embeds_det, masks
...
class RTDETRTransformerMOT_v2(RTDETRTransformer):
    ...
    def __init__(self,
        ...
        self.dec_mask_head_track = nn.ModuleList([
          MLP(hidden_dim, hidden_dim, 300, 3) for _ in range(num_decoder_layers)  
        ])
    ...
    def forward(self, feats, targets=None, track_queries=None, ind_track=True, criterion= None, mode='', limit=2, update_track=False): #limit=10 or 30 -> cuidado con el limite
        ...
        #Generate mask for detections
        th = 0.5
        scores = F.sigmoid(out_logits[-1])
        mask = torch.max(scores,2)[0] > th

        #Add first predictions
        if 'embeds_2' not in track_queries and mask.sum()>0:
            track_queries['embeds_2'] = track_queries['hs'][mask.clone()].unsqueeze(0)
            track_queries['pred_boxes_2'] = out_bboxes[-1][mask.clone()].unsqueeze(0)
            track_queries['pred_logits_2'] = out_logits[-1][mask.clone()].unsqueeze(0)
        ...
        if 'embeds_2' in track_queries:
            print('embeds 2 shape:',track_queries['embeds_2'].shape)
            aux_boxes = torch.cat((track_queries['pred_boxes_2'],out['pred_boxes']),dim=1)
            aux_embeds, aux_boxes, aux_logits, last_attn_weights, self_attn, track_queries['embeds_track'], track_queries['embeds_det'], track_queries['masks'] = self.decoder_track(torch.cat((track_queries['embeds_2'],track_queries['hs']),dim=1),self.query_pos_head(aux_boxes),aux_boxes,track_queries['hs'],None,self.dec_bbox_head_track,self.dec_score_head_track,
                self.dec_mask_head_track)
            track_queries['embeds_2'] = torch.split(aux_embeds,[track_queries['embeds_2'].shape[1]])
            track_queries['pred_boxes_2'] = torch.split(aux_boxes,[track_queries['embeds_2'].shape[1]])
            track_queries['pred_logits_2'] = torch.split(aux_logits,[track_queries['embeds_2'].shape[1]])
        ...
        if 'embeds_2' in track_queries: 
            ...
            mask[:,torch.unique(torch.nonzero(mask[0].clone())[sim_idxs])] = False
            track_mask  = torch.max(torch.sigmoid(track_queries['pred_logits_2']),2)[0] > 0.5 #probando
            ...
            if track_queries['embeds_2'].shape[1] > 0:
                if "id_max" not in track_queries:
                    ...
                    track_queries["ids"] = torch.cat((track_queries["ids"],torch.arange(id_max+1,id_max+1+mask.clone().sum()).to(track_queries["embeds_2"].device)))
